...
function onJsonSerie(json)
{
    console.log(json);
    const view=document.querySelector('#serie_view');
    view.innerHTML='';
    ...
    for(let i=0;i<6;i++)
    {
        ...
        img.src=doc.image;
        const voto = document.createElement('p');
        voto.textContent=doc.imDbRating;
...
{
    console.log(json);
    const view=document.querySelector('#film_view');
    view.innerHTML='';
...
fetch('imdb_popular_serie.php').then(onResponse).then(onJsonSerie);
fetch('imdb_popular_film.php').then(onResponse).then(onJsonFilm);
